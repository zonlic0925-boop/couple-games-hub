...
| **大厅 UI 视觉设计** | 🟢 已重设计 | 暖粉光斑氛围底、毛玻璃卡片/分类胶囊/底部抽屉、单一玫瑰强调色、图标内高光、桌面 hover 浮起反馈 |
...
5. **iPhone 灵动岛遮挡退出按钮**（上一轮）：DualCourt 乒乓/网球顶栏、四子棋屏幕、反应王浮动按钮此前无 Safe-Area 顶部内边距，`viewport-fit=cover` 下被灵动岛/状态栏压住无法退出；统一补 `calc(env(safe-area-inset-top, 20px) + Npx)`（四子棋并补 flex column + 底部安全区），已在 393×852 模拟灵动岛 59px 下实测按钮顶 69px 且退出闭环成功。
6. **大厅 UI 重设计**（本轮，按 redesign-existing-projects skill）：主界面观感从「白卡 + 黑投影 + 杂色徽章 + 浅粉平底」升级为「暖粉光斑氛围底 + 毛玻璃悬浮卡片 + 单一玫瑰强调色 + 胶囊分类条」；头部加品牌 logo 与在线状态胶囊（修正"20 款"为 21 款文案）；21 个游戏徽章移除五颜六色内联底色统一为柔和玫瑰标签；图标加玻璃高光；模式选择改毛玻璃底部抽屉、三模式配暖色系卡片；配对面板背景并入暖粉氛围；桌面 hover 浮起 + 按压 scale 反馈。
